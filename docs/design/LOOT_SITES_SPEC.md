...
| `GameState.gd` | `gold` as the sixth resource (all four resource methods). Nothing else — loot never touches GameState until the deposit step, which is the other spec's business. **Corrected 2026-08-30:** "nothing else" predates this document's own 2026-08-29 amendment ruling 2, which added `arms`; a kind he can carry but cannot bank would make every deposit containing weapons print `unknown kind` and drop them, so `arms` is the seventh resource, banked and inert until `COMBAT_SPEC.md` §9's gear v1. |
