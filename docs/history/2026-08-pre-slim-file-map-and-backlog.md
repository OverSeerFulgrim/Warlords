...
- ~~measure_travel (fixed 2026-08-05): lair -> nearby resource is 3s vs the 10-20s design target - retune resource placement or speeds before R2.~~ **Resolved 2026-08-27 (prompt U1), and nothing was retuned.** It was a harness category error: the row judged the lair's own worker nodes - close by design, workers walk them every trip - against a target written for sortie-scale resources. The 3s specifically was the *roaming deer* wandering through `ResourceField.nodes`. The harness now reports the two as separate rows and fixes its seed; the sortie row is PENDING R2a with the 10-20 cell ring asserted walkable and in band. See `2026-08-world-population-r1.md`.
